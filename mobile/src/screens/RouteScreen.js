...
export default function RouteScreen({ scenario, result, deliveredIds, toggleDelivered }) {
  ...
  const currentStop = navState === 'navigating' ? stops[currentStopIndex] : stops.find(s => !deliveredIds.has(s.client.client_id))
  ...
  function handleDelivered() {
    ...
    toggleDelivered(stop.client.client_id)
            ...
            <View style={styles.mapWrapper}>
              ...
              >
                ...
                <Polyline
                  ...
                  strokeColor="#1e293b"
                  strokeWidth={3}
                  lineDashPattern={routeLegs.length > 0 ? undefined : [8, 5]}
                />
                {sampleArrows(previewPolyline).map((arrow, i) => (
                  <Marker
                    key={`arrow-${i}`}
...
const styles = StyleSheet.create({
  ...
  progressLabel: { fontSize: 11, fontWeight: '600', color: '#94a3b8', letterSpacing: 0.8, textTransform: 'uppercase' },
  ...
  stopRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 16, padding: 14, gap: 12 },
  stopRowCurrent: {
    borderWidth: 1.5,
    borderColor: '#3b82f6',
    shadowColor: '#3b82f6',
  ...
  deliverBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, backgroundColor: '#64748b', flexShrink: 0 },
  ...
  navStopBadge: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 6 },
